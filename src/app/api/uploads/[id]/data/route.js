...
export function createDatabaseUploadPut(database = db) {
  return async (req, { user, params }) => {
    const { id } = await params;
    const asset = await database.mediaAsset.findFirst({
      where: { id, ownerId: user.id, storageProvider: "DATABASE", deletedAt: null, verifiedAt: null },
      select: { id: true, bytes: true, mimeType: true },
    });
    if (!asset) return NextResponse.json({ error: "Upload asset not found" }, { status: 404 });
    if (asset.bytes > MAX_BYTES) return NextResponse.json({ error: "Database fallback images must be 4 MiB or smaller" }, { status: 413 });
    ...
    const updated = await database.mediaAsset.updateMany({
      where: { id: asset.id, ownerId: user.id, storageProvider: "DATABASE", deletedAt: null, verifiedAt: null },
...
export const PUT = withAuth(createDatabaseUploadPut());
