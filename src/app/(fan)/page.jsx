    ...
    <section className="mt-8">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {viewAll && (
            <button className="text-[13px] font-semibold text-brand-600 hover:underline cursor-pointer">
              View All
            </button>
          )}
          <button className="grid h-8 w-8 place-items-center rounded-full border border-line bg-white text-muted hover:bg-canvas cursor-pointer">
            <ChevronLeft size={15} />
          </button>
          <button className="grid h-8 w-8 place-items-center rounded-full border border-line bg-white text-muted hover:bg-canvas cursor-pointer">
    ...
    <div className="flex gap-6 px-6 py-6 xl:pr-6">
      <div className="min-w-0 flex-1">
        <div className="flex items-start justify-between">
          <div>
            <h1 className="text-[25px] font-extrabold tracking-tight">
              Good morning, {user?.name ? user.name.split(" ")[0] : "Arjun"}! 👋
            </h1>
            <p className="mt-1 text-[14px] text-muted">
              Ready to discover something amazing today?
            </p>
          </div>
          <button className="flex h-10 items-center gap-2 rounded-xl border border-line bg-white px-4 text-[13.5px] font-semibold hover:bg-white/70 cursor-pointer">
        ...
        <Card className="mt-4 flex items-stretch gap-1 overflow-x-auto px-3 py-3 no-scrollbar">
          {quickLinks.map(({ icon: Icon, tint, title, sub }) => (
            <button
              key={title}
              className="flex min-w-[168px] flex-1 items-center gap-3 rounded-xl px-3 py-2 text-left hover:bg-canvas cursor-pointer"
            >
      ...
      <aside className="hidden w-[320px] shrink-0 space-y-4 xl:block">
          ...
          <div className="mt-3 space-y-3.5">
            {creators.slice(0, 4).map((c) => (
              <Link
                key={c.name}
                href={`/creator/${slug(c.name)}`}
                className="flex items-center gap-3 hover:bg-canvas p-1 rounded-xl transition"
              >
          ...
          <div className="mt-3 space-y-3">
            {liveSessions.map((s, i) => (
              <div key={i} className="flex items-center gap-3">
                ...
                <button className="h-7 shrink-0 rounded-lg border border-brand-200 px-2.5 text-[11.5px] font-bold text-brand-700 hover:bg-brand-50 cursor-pointer">
                  Remind
                </button>
              </div>
            ))}
        ...
        <Card className="p-4">
          ...
          <div className="mt-3 flex items-center gap-3">
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between text-[12px]">
                <span className="font-semibold text-ink">Level {user?.level || 3}</span>
                <span className="font-semibold text-muted">
                  <b className="text-ink">{currentXp}</b> / {currentLevelXpNeeded} XP
                </span>
              </div>
          ...
          <div className="mt-4 space-y-3">
            {rewardsList.map(({ icon: Icon, label, xp, progress }) => (
        ...
        <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-brand-600 via-[#8b5cf6] to-[#e05fd6] p-5 text-white">
          ...
          <button className="mt-3.5 h-8 rounded-lg bg-white px-4 text-[12.5px] font-bold text-brand-700 cursor-pointer">
            Invite Now
          </button>
          <Gift
            size={110}
