...
export default function Rewards() {
  ...
  const [tasks, setTasks] = useState([
    { id: 1, label: "Daily Login", xp: 50, progress: null, completed: true, claimed: true },
    { id: 2, label: "Watch 3 Live Streams", xp: 100, progress: "2 / 3", completed: false, claimed: false },
    { id: 3, label: "Refer a Friend", xp: 200, progress: "0 / 1", completed: false, claimed: false },
    { id: 4, label: "Like 5 posts in Feed", xp: 75, progress: "5 / 5", completed: true, claimed: false },
    { id: 5, label: "Send a Tip to Creator", xp: 150, progress: "0 / 1", completed: false, claimed: false },
  ]);
  ...
  const fetchUserData = () => {
    fetch("/api/auth/me")
      ...
      .catch((err) => console.error("Error loading user state in rewards:", err));
  };
  ...
  const handleClaim = async (id, xpVal) => {
    try {
      const response = await fetch("/api/rewards", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          questId: tasks.find((t) => t.id === id)?.label || "Quest Reward",
          xpReward: xpVal,
        }),
      });

      if (!response.ok) {
        throw new Error("Failed to claim reward via API");
      }
      ...
      // Sync state and fire updates
      fetchUserData();
      window.dispatchEvent(new Event("user-update"));
      window.dispatchEvent(new Event("notifications-update"));

      if (result.didLevelUp) {
        alert(`Congratulations! You leveled up to Level ${result.level}! 🎉`);
      } else {
        alert(`Successfully claimed +${xpVal} XP!`);
      }
    } catch (error) {
      console.error("Claim Reward Error:", error);
      alert("Something went wrong while claiming reward.");
    }
  };

  const currentLevelXpNeeded = user ? user.level * 1000 : 1000;
  const currentXp = user ? user.xp : 650;
  const progressPct = Math.min(100, (currentXp / currentLevelXpNeeded) * 100);
    ...
    <div className="flex flex-col xl:flex-row gap-6 px-6 py-6 min-h-[calc(100vh-72px)] bg-canvas">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between text-[14px]">
                <span className="font-extrabold uppercase tracking-wider text-white/90">
                  Level {user?.level || 3} – Super Fan
                </span>
                <span className="font-black text-white">
                  {currentXp} / {currentLevelXpNeeded} XP
                </span>
              </div>
              ...
              <div key={t.id} className="p-4 flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  <span className={`grid h-7 w-7 place-items-center rounded-full ${
                    t.completed || t.claimed ? "bg-emerald-50 text-emerald-600" : "bg-neutral-50 text-muted"
                  }`}>
                ...
                <div className="flex items-center gap-4">
                  <span className="text-[13px] font-extrabold text-brand-600">+{t.xp} XP</span>
                  {t.completed && !t.claimed ? (
                    <button
                      onClick={() => handleClaim(t.id, t.xp)}
                      className="h-8 rounded-lg bg-brand-600 px-3.5 text-[12px] font-bold text-white hover:bg-brand-700 shadow cursor-pointer"
                    >
                      Claim
                    </button>
                  ) : t.claimed ? (
      ...
      <aside className="w-full xl:w-[320px] shrink-0">
        <Card className="p-5">
          ...
          <button className="w-full mt-4 h-9 rounded-xl border border-line text-[12.5px] font-bold text-ink hover:bg-canvas cursor-pointer">
